...
function parseUsers(): User[] {
  ...
  // ✅ DEBUG exact (dans le terminal où tu fais npm run dev)
  console.log("SW_LOGIN_USERS =", raw);
...
export async function POST(req: Request) {
  try {
    ...
    const isValid = users.some((u) => u.username === username && u.password === password);
    ...
    const msg = e instanceof Error ? e.message : "Erreur configuration";
    return NextResponse.json({ ok: false, error: `Service indisponible (configuration). ${msg}` }, { status: 503 });
  }
}
